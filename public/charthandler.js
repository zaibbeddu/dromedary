...
dromedaryChartHandler = function () {
  ...
  $.getJSON(apiBaseurl+'/feature-toggles', {}, function(featureFlags, status) {
    ...
    // liatriofy toggle
    if (featureFlags['liatriofy'] === true) {
      $('#logo-link').attr('href', 'http://liatrio.com');
      $('.logo').attr('alt', 'Liatrio');
    } else {
      $('#logo-link').attr('href', 'http://stelligent.com/');
      $('.logo').attr('alt', 'Stelligent Systems');
    }
  });
